...
- **Nested-property predicate filtering (`where=`)** on both `AssertionStore.match` and `match_across` — a single comparison against one named nested property (e.g. `where=(CONFIDENCE_REL, '>', 0.8)`), not a general filter language. Additive/backward-compatible on `match` (default `None` = unconstrained). (#32)
