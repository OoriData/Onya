...
- **`onya.store.OverlayReadStore`** — read-time union/scoped access across multiple named graphs, without disturbing per-graph storage: `union(names)` (fully materialized merged view, reproducing SPEC merge Rules 1-3 exactly as an in-memory `graph.union()` would — not a bare row concatenation), `match_across(names, ...)`, `subgraph_across(names, roots, hops)`. SQLite and PostgreSQL implement it via genuine `WHERE graph_pk IN (...)` pushdown (a small constant number of round trips, not `len(names)`); the filesystem backend does not implement it (no pushdown is possible there — same reason it doesn't implement `AssertionStore`), and documents the `get()` + `graph.union()` fallback. `union()` raises `GraphMergeError` on a genuine cross-graph Rule 1 conflict, `KeyError` for an absent name, `ValueError` for an empty name list. (#32)
- **Nested-property predicate filtering (`where=`)** on both `AssertionStore.match` and `match_across` — a single comparison against a nested property *at any depth* (e.g. `where=(CONFIDENCE_REL, '>', 0.8)`, matching *any* candidate at any nesting level, not just direct children), not a general filter language. Deliberately not depth-limited: `@confidence` nests under `@method`, itself nested under the fact it corroborates (#36) — a direct-children-only search would silently never find it, which is exactly the shape the motivating example uses. Additive/backward-compatible on `match` (default `None` = unconstrained). (#32)
- **`onya.store.sync.SyncStore` now wraps `AssertionStore` and `OverlayReadStore`** (`match`, `subgraph`, `add`, `remove`, `union`, `match_across`, `subgraph_across`, `overlay`), not just `GraphStore`'s floor as before. Each capability's methods are bound as instance attributes only when the wrapped store actually offers it, so `isinstance(sync_store, AssertionStore)` (etc.) reports the same capability the underlying async store has — a filesystem-backed `SyncStore` still correctly reports neither. (#32, #38)
- **`assertions_mixin.any_prop_value(label, default=None)` / `any_edge_target(label, default=None)`** (`pylib/graph.py`) — the "raw value with a default" shortcut `getprop`/`getedge` deliberately don't provide (they stay multi-value generators, correctly so). Returns the property's string value / edge's target directly rather than the assertion object, since `next(self.getprop(label), None)` already covers the object-chaining case (nested properties, `.interp`, `.id`) with no new API needed. An arbitrary pick when more than one assertion matches — `properties`/`edges` are sets, so "first" would overstate an ordering guarantee the model doesn't make; well-defined when there's at most one match, the intended use.
- **`OverlayReadStore.overlay()`** — `union()`'s more opinionated sibling: named graphs treated as ordered layers, resolving a genuine same-label disagreement to one winner instead of keeping every value. `single_cardinality` (a collection of assertion labels, or a callable `label -> bool`, for arbitrary logic) is the caller's own cardinality declaration — Onya's core model has no schema to consult, so nothing shadows by default and `overlay()` with no hints reduces to `union()`'s behavior. A genuine `@id` conflict is *always* resolved here (never raises, unlike `union()`) since an explicit `@id` already declares single-occurrence intent. `key` picks the winner via `max(candidates, key=key)` — "be like Python sorting," so arbitrary per-label/per-node precedence logic is just a function, no bespoke override-policy object; `precedence` is sugar for the common list-order case, and `prefer_confidence=True` swaps in a default key that prefers a candidate's resolved `@confidence` (via #36) before falling back to precedence. Losing assertions are removed from the returned graph, not left in it tagged — the audit trail is the returned `list[ShadowedConflict]` (winner, losers, and per-candidate `key_values` for debugging), never silently dropped. (#38)
...
- **`literate.write` no longer emits silently non-round-tripping output for a separator-less namespace base.** `write` compacts IRIs under a declared `@schema`/`@nodebase` to bare names, but bare names re-expand by *pure concatenation* on reparse — so a base lacking a trailing separator (e.g. `https://example.org/vocab`, an easy slip) minted mashed IRIs like `…/vocabtitle` instead of `…/vocab/title`, with no error and no warning. `write` now emits self-consistent, separator-terminated bases: a separator-less `schema`/`nodebase` is normalized (append `/`) with a `UserWarning` — parity with the parser's existing read-side check — or raises `NamespaceBaseError` under `write(..., strict_namespace_bases=True)`. `write` is a faithful inverse of `read` under *any* namespace arguments now (unmatched IRIs already fall back to explicit `<full-iri>`); the separator is the one precondition. Documented as a round-trip guarantee in SPEC § Serialization.
