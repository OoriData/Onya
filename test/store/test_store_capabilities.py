...
backend is a ``GraphStore``; the filesystem backend is *not* an ``AssertionStore`` or an
``OverlayReadStore`` (both would be a lie — the file must be parsed whole); SQLite is an
``AssertionStore`` and an ``OverlayReadStore`` but not a ``GraphQueryStore``.
