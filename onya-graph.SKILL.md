...
- **Showing a node.** Don't hand-write per-type renderers; declare a view: `view.project(g, node, view.load(toml_data))` with specs like `{type='schema:Person', fields=['name', {label='email', many=true}], follow=[{edge='worksFor', as='employer', show=['name'], edge_props=['startDate']}, {inbound='author', as='works', show=['name'], order_by='-datePublished', limit=10}]}`. Output is ordered plain data (`fields` as `(name, value)` pairs); `many=false` picks the least value deterministically; missing data is omitted, never an error. `view.project_from_store(store, name, node_id, specs, prefixes=g.prefixes)` fetches only the needed neighborhood.
